...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($csrf, $in_csrf)) {
        ...
        if ($display_name === '') {
            ...
            } elseif (!empty($_FILES['logo']) && $_FILES['logo']['error'] !== UPLOAD_ERR_OK && $_FILES['logo']['error'] !== UPLOAD_ERR_NO_FILE) {
                $flash_error = 'Logo yükleme hatası (kod: ' . (int) $_FILES['logo']['error'] . ').';
            }

            if ($flash_error === null) {
                try {
                    $sql = "UPDATE teams SET display_name = :d, contact_email = :e, website = :w, bio = :b, logo = :l, instagram_url = :iu, youtube_url = :yu, linkedin_url = :li WHERE id = :id";
                    $upd = $pdo->prepare($sql);
                    $upd->execute([
                        ':d' => $display_name,
                        ':e' => $contact_email,
