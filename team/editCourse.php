...
$ytValue = $isYT ? $course['intro_video_url'] : '';              // YT input sadece YT gösterir
    ...
    <form id="meta-form" class="section-card grid grid-cols-1 lg:grid-cols-2 gap-5" method="POST" action="course_update_meta.php">
        ...
        <div class="space-y-4">
            ...
            <div class="input-card">
                ...
                <label for="comp" class="font-semibold text-gray-800">Yarışma Türü</label>
...
<script>
    ...
    function setYouTubePreview(videoId) {
        ...
        if (videoId) {
            ...
            hidden.value = 'https://youtu.be/' + videoId; // DB için canonical

            err.style.display = 'none';



            // YouTube aktif -> dosya UI kapanır + video src temizlenir
            ...
            // YouTube kapandı; yerel video yoksa drop zone aç
    ...
    function setupDropZone(o){
        ...
        function upload(file) {
            ...
            // 💡 Kaydetme butonlarını devre dışı bırak
            const saveBtns = document.querySelectorAll('button[type="submit"]');
            saveBtns.forEach(btn => {
                btn.disabled = true;
                btn.classList.add('opacity-60', 'cursor-not-allowed');
                btn.innerHTML = '<i data-lucide="loader-2" class="animate-spin"></i> Video yükleniyor...';
            });
            lucide.createIcons();

            // HEMEN gizle (küçük dosyada progress gelmeyebilir)
            ...
            xhr.onload = function () {
                // ✅ Yükleme tamamlandığında butonları tekrar aktif et
                saveBtns.forEach(btn => {
                    btn.disabled = false;
                    btn.classList.remove('opacity-60', 'cursor-not-allowed');
                    btn.innerHTML = '<i data-lucide="save"></i> Değişiklikleri Kaydet';
                });
                lucide.createIcons();
                ...
                if (xhr.status === 200) {
                    try {
                        ...
                        if (r.success) {
                            // 1) DB’ye gidecek hidden alanı güncelle (dosya yolu)
                            if (o.uploadType === 'intro') {
                                const hidden = document.getElementById('intro_video_url');
                                if (hidden && r.url) hidden.value = r.url;

                                // 2) YouTube önizlemesini gizle
            ...
            xhr.onerror = function () {
                // ❌ Hata durumunda da butonları tekrar aktif et
                saveBtns.forEach(btn => {
                    btn.disabled = false;
                    btn.classList.remove('opacity-60', 'cursor-not-allowed');
                    btn.innerHTML = '<i data-lucide="save"></i> Değişiklikleri Kaydet';
    ...
    function resetYouTubeSelection() {
        ...
        // 4) Hidden'ı sadece şu durumda boşalt:

        //    - hidden YT ise (YT’yi gerçekten temizliyoruz), veya

        //    - hiç yerel önizleme yoksa
        ...
        // 5) Yerel önizleme yoksa dropzone’u aç
